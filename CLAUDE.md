...
- **Email pipeline (Resend)**: single Resend account on the verified `landadvisors.com` domain serves both pipelines. Feedback notifications send from `no-reply@landadvisors.com` (via `EMAIL_FROM`) so recipients know replies aren't monitored. Client-facing sends (OM blast + Deal Team) use a hardcoded `cshiota@landadvisors.com` from the composer's sender dropdown, passed as a per-call `from` override. `sendBlastEmails` server action fetches selected attachments from Vercel Blob, then calls Resend per builder/team with partial-failure reporting back to the composer toast.
