...
- **PDFs shipped**: Marketing Report (per-builder tier + comments), Q&A File (approved items), Due Diligence Tracking (milestones + issues + deal team + consultants; replaced the standalone Issues Report), Deal Status, Consultant Roster (13 roles with firm / side / contact, plus a not-yet-engaged list). Land Advisors branding via bundled Metropolis font + the real LAO logo image.
...
- **Email pipeline (Resend)**: single Resend account on the verified `landadvisors.com` domain serves both pipelines. Feedback notifications send from `no-reply@landadvisors.com` (via `EMAIL_FROM`) so recipients know replies aren't monitored. Client-facing sends (OM blast + Deal Team + Phase 2 buyer blasts) use a hardcoded `cshiota@landadvisors.com` from the composer's sender dropdown, passed as a per-call `from` override. `sendBlastEmails` server action fetches selected attachments from Vercel Blob (and renders generated PDFs in-process when attached), then calls Resend per builder/team with partial-failure reporting back to the composer toast. Every client-facing send BCCs the sender so a copy lands in their inbox (Resend doesn't post to Outlook Sent Items). Row-button sends that exist to ship a document (Market Study, DD folder) gate the composer behind a pre-flight check via `requireAttachment="file" | "any"`; rejection surfaces an inline red bubble anchored to the button (see `src/components/inline-error-bubble.tsx` for the reusable pattern).
