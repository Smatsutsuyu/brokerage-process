...
- **Step 2 done 2026-08-03**: the Phase 1 row reads and writes the roster, and migration `0034` backfills the legacy columns across. The backfill moved forward from step 3 into step 2 because a production dry-run showed the read change alone would hide two deals' attorneys until a later deploy.
- **Step 3 remaining**: drop `psa_attorney_name` and `psa_attorney_firm`. Pure cleanup now, with nothing left to preserve. Should wait until step 2 has run in production long enough to be confident, and until the backfilled rows have been eyeballed on the Consultants tab.
