...
- **Trailing row dividers removed** across all three PDFs. Every data row carried a bottom border, so the last row of a group drew a rule against the next band, the next section header, or nothing at all. Dividers now only separate rows *within* a group; a single-entry role renders with no rule.
...
- **Coverage is now effectively complete.** Every server action that mutates shared data writes an entry, across eleven action files. Verified mechanically rather than by inspection: a checker maps every function containing a `db` or `tx` write (including inside `db.transaction`) against `write*Audit` calls, and reports 0 missing, 69 covered, 5 excluded by policy.
- **Deliberate exclusions, unchanged in spirit from the checklist batch**: per-user sidebar deal ordering, `setMyNotificationPreference`, and `submitFeedback` / `addFeedbackComment` / `recordFeedbackAttachment`, whose rows already carry author and timestamp.
- **One exclusion was reversed on review.** `editFeedbackComment` was initially excluded on the "the row carries its own attribution" reasoning. Two independent reviewers established the row names the AUTHOR, not the EDITOR, that an owner may edit anyone's comment, and that no history table exists, so the prior text was unrecoverable with no trace of who rewrote it. Now audited as destructive.
- **`banner-actions.ts` was missing from the backlog's inventory entirely.** Two mutations, one of which permanently deletes a blob. Found by enumerating `"use server"` files rather than trusting the list.
- **`auditSafely()` added to `src/lib/audit.ts`.** `writeAudit` swallows its own errors, but that only covers the INSERT. Roughly a dozen audit-only SELECTs had landed on the critical path *after* their mutation committed, where a throw would make the action fail for a write that already succeeded. The worst was `recordUpload`: the client renders that as "Upload failed" and skips its refresh, so the user is told an upload failed while the document and blob both exist. Post-mutation audit work now goes through the helper. Pre-mutation snapshots deliberately do not, since those fail closed.
- **`recordUploadedDocument` now returns the inserted row.** The audit entry previously re-queried for "the newest row matching name + checklist item", which is a guess: re-uploading the same filename is the normal replace flow, so concurrent replaces attributed the wrong row's id, version and size.
- **Bulk entries no longer put a deal UUID under a row-level `entityType`.** `entity_id` is UUID-typed, so it did not error, which is what made it dangerous: the viewer printed an id labelled `deal_buyer` matching no buyer row. Now null, with the deal identity in `metadata` where the filter reads it anyway.
- **Viewer grew a Type filter and a Deletions filter.** With ~90 action strings a single Action dropdown is unusable, so picking a Type narrows it. Deletions get their own filter and red treatment because for a destroyed row the entry is the only surviving record. Labels cover ~90 actions and 17 entity types, still with a prettifier fallback so a later action renders sensibly before anyone updates the map.
- **Process note.** The sweep ran as three multi-agent passes: apply, adversarially verify, fix. Verification found 41 issues in the first pass (15 major) and 11 more after the fixes, including the `auditSafely` class of defect, which no single-pass review would have surfaced. Three separate comments were caught claiming an entry preserved fields its snapshot never selected.
...
- **One email instead of three.** The Phase 4 Share DD Material and Complete Due Diligence rows now send a single email: To = Owner + Buyer Team, CC = Broker Team (pre-checked) + the marketing coordinator. Previously the composer emitted one email per sub-team behind a paginator, which Chris read as "it only sends to ownership".
- **New opt-in component**, `views/unified-deal-team-send-button.tsx`, not a flag on `DealTeamSendButton`. Schedule SOO Review deliberately keeps the old per-sub-team behavior (Excel specs it Owner + Broker; CC'ing the eventual buyer on an offer-review invite would be wrong). Reverting a row = swapping which component it renders.
- **New action `getUnifiedDealTeamComposerData`** rather than widening the three shared recipient actions, which the OM blast and three two-step composers also use.
- **Shaping lives in `src/lib/email/unified-deal-team.ts`** — `actions.ts` is `"use server"` and may only export async functions, so logic inside it can't be exercised by a script. The action is queries-only; `npm run verify:unified-composer` feeds the same pure transform real rows (30 assertions, 4 scenarios, localhost-guarded).
- **Recipient provenance caps** on To/CC chips: a leading token showing which side each address sits on, plus the person's role as a dim suffix. Additive optional props on the shared composer, so the other five callers are byte-identical. Vocabulary is quoted from existing surfaces (deal team sub-team labels, the Consultants tab side badge, the CC picker's "Org Members") rather than newly coined.
- **Consultants are CC-able**, grouped by side, never pre-checked; ones with a missing or unparseable address render disabled with the reason rather than vanishing.
- **Broker fallback**: when nobody on ownership or the buyer side has an address, the brokerage takes the To line and the composer says so. When nobody at all is addressable, the button rejects at click time with the inline bubble instead of opening an unsendable composer.
- **Three fixes to shared code pulled in along the way**: the blast CC-persistence filter became an allowlist (a prefix denylist would have let the new `consultant:` ids reach a `uuid[]` column); `handleSend` now catches a thrown send instead of failing silently for all five composers; `updateConsultant` validates through `parseEmailAddress` like `addConsultant` already did.
- **DD Tracking now attaches as real bytes.** Surfaced by the review and fixed in the same batch: the Complete Due Diligence row attached its report as `kind: "link"` with a relative, session-gated URL, which `appendLinksToBody` pasted into the message as literal text — that send had never delivered the report. Extracted `generateDdTrackingPdf` into `src/lib/pdf/generate-dd-tracking.ts` (pure move of the route loader), refactored the route to a thin wrapper, wired the previously-throwing `dd-tracking` key in the generator registry, and switched the row to `kind: "generated"`. Four of four in-app PDFs can now be attached to a blast.
...
- **Audit log retention.** `audit_log` is append-only and has no trim job. CLAUDE.md records a one-year working assumption; trimming is a manual DELETE documented in `docs/operations.md`. Worth revisiting once there is real volume.
