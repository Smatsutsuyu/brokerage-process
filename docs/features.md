...
Every item supports a completion checkbox (with timestamp and who completed it — completed items show a compact "Chris · Jul 20" next to the strikethrough title, and hovering that label reveals the full date + time), inline notes, file attachments stored on Vercel Blob (replacing a file auto-versions the prior copy), and an external link slot for Dropbox folders or any URL. Unchecking a completed item prompts a confirmation dialog because it clears the completion timestamp and completer record.

Some items have extra affordances:

- Ten items across Phases 2, 3 and 4 carry milestone dates: Offering Date, Schedule Summary of Offer Review, Send out B&amp;F, and the seven Phase 4 milestones (LOI Signed, PSA Effective, the two Cost to Complete dates, Investment Committee Approval, Waive Feasibility, Closing Date). Each row shows **one** date plus an **Est.** checkbox. Leave Est. unticked and the date is firm, which may be a date in the future such as a closing. Tick it and the date reads as a projection and turns amber. Tick the item's own checkbox and the date reads **Completed**. Adding a date opens the picker on today, so recording a milestone the day it lands is click and confirm, while a scheduled date is one pick away. Unticking Est. stamps today by default: that is the moment a projection becomes a real date, and the original projection is kept behind the scenes so the slip between them still shows up in reports. Clear the date from the picker to remove it; clearing a firm date falls back to the projection if there was one.
...
- Some items have inline action buttons that share behavior with the relevant tab. Wired and working today: Send OM Blast on the OM Blast row, Send Q&A File on the Q&A File row, Share Market Study on its row, Send CTC on the Phase 1 Cost to Complete (CTC) row, Send to Deal Team on Share Due Diligence Material and on Complete Due Diligence, Marketing Report PDF on its row, Send Marketing Report on the Phase 2 row of the same name (two-step modal: preview the freshly-generated PDF, then compose the email to the Owner Team with a CC picker), the inline PSA Attorney picker on Determine PSA Attorney, Notify PSA Attorney on the Phase 4 Kick off PSA row, and Schedule meetings on the Phase 4 Schedule Recurring Call row. CFD Analysis, Premium Analysis, Valuation, Entitlement Schedule, Entitlement Summary, Custom Underwriting File, and Compiled Package still surface "Coming soon" placeholder toasts and are scoped for a future phase.
...
A living issues tracker for due diligence. Add an issue with a title, description, priority (low / medium / high / urgent), assignee, status (open / in-progress / resolved), and identified/resolved dates. The assignee picker lists every Deal Team member on this deal across all three sub-teams (Owner / Broker / Buyer), regardless of identity source — an org user, an external contact, or a free-text name all show up as valid assignees. The dropdown groups by sub-team with section headers so it's clear who sits where. An assignee since removed from the team still appears in the picker under "Other" so editing an existing issue doesn't drop them. Unassigned issues show a highlighted "Unassigned" pill on the card to nudge you to pick someone. The tab badge shows the open count. Two toolbar exports: Generate PDF opens the Due Diligence Tracking report (combined milestone dates + issues + deal team + consultants) for your DD calls; Export to Excel downloads a single-sheet workbook of every issue on the deal (title, description, status, priority, assignee, identified/resolved dates). The DD Tracking PDF shows the deal's Final Purchase Price at the top when set (edit via the deal-header Edit button), and prefixes any milestone that has happened (checklist item checked, OR tracked date is on/before today) with a ✓ so past events are visible at a glance. The same purchase price shows as an emerald chip next to the deal name at the top of every tab of the deal page, so it stays top-of-mind while you're working. When the price is unset, the chip slot renders a dashed "+ Add purchase price" button that opens the deal edit form directly.

### Deal header actions

The row above the tabs surfaces three action buttons: **Email Status** (opens a two-step modal that previews the Status Report PDF then composes an email to the Owner Team with the PDF attached; content: current phase, progress by phase, recently completed items with attribution, upcoming milestones, open issues, Owner Team roster), **Edit** (opens the deal edit form directly — same as clicking the "+ Add purchase price" CTA), and a small **⋯** overflow menu for less-frequent actions (Set/Change banner image, Archive/Unarchive, Delete). Delete only appears on archived deals — the archive-then-delete two-step means you can never destroy a deal in one click.

### Archive and delete

Archiving a deal hides it from the sidebar (moved under a collapsible "Archived (N)" section at the bottom) and removes it from the priority ribbon even if starred. The deal page itself still works when archived, showing an "Archived" pill in the header — you can view everything, and the ⋯ menu now shows "Unarchive deal" and "Delete deal". Archive is a one-click reversible action (with a soft confirmation). Delete is destructive and irreversible, gated behind archive, gated behind its own destructive confirm; the server action refuses to delete an active deal even if called directly.
...
- Due Diligence Tracking. Combined PDF for the bi-weekly DD call: the 7 milestone dates from the Phase 4 checklist, current issues grouped by status (no summary stats), the full Deal Team (Owner / Broker / Buyer subteams), and the consultant roster. Generated from the Issues tab toolbar or the Phase 4 "Complete Due Diligence" row. Emailing it from that row runs the same two-step flow as the Marketing Report, Consultant Roster, and Deal Status sends: review the freshly-generated PDF, then continue to the composer with it already attached. The preview and the attachment render from the same loader, so what you approve is what recipients receive, as a real file rather than a link they'd have to log in to open.
