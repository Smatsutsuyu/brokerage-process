...
- Ten items across Phases 2, 3 and 4 carry milestone dates: Offering Date, Schedule Summary of Offer Review, Send out B&amp;F, and the seven Phase 4 milestones (LOI Signed, PSA Effective, the two Cost to Complete dates, Investment Committee Approval, Waive Feasibility, Closing Date). Each shows **two** date chips, not one: an amber **Est.** for the date you are projecting, and a blue **Actual** for the date it landed. They are independent, so a row can hold both and the gap between them is the slip. Est. opens the date picker, since a projection is rarely today; Actual stamps today in one click, which is how you use it when a milestone lands. Clear either one from the picker to remove it. Every date that existed before this shipped was moved into Est., since those were projections; Actual starts empty and fills in as milestones land.
