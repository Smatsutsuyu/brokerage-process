...
| `npm run verify:psa-resolution` | Assert the PSA attorney resolver + Kick off PSA composer shaping (pure fixtures, no DB) |
...
`npm run verify:psa-resolution` needs no database at all. The PSA attorney resolver is a pure function over consultant rows, so this runs on fixtures covering all five resolution states, address repair and rejection, To-line dedupe, the derived drafting sentence, and the CC pool. Run it after touching `src/lib/psa-attorney.ts`.
