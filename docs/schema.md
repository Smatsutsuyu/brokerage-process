...
- **`deals`** (`src/db/schema/deals.ts`) — one row per brokerage deal. Columns: `name`, `units`, `city`, `state`, `type`, `priority` (`deal_priority` enum), `purchase_price` (nullable `numeric(14,2)` — final dollar value, distinct from the Phase 4 "Finalize CTC / Final Purchase Price" checklist milestone which is the DATE), `archived_at` (nullable timestamp — non-null hides the deal from the sidebar + priority ribbon; required non-null before `deleteDeal` will accept the row, enforcing an archive-then-delete two-step), `notes`, and `psa_drafting`. **`psa_attorney_name` and `psa_attorney_firm` were dropped in migration `0036`**: PSA attorney contact details now live on the consultant roster as a `psa_attorney` row, which is the only place that can hold an email address. Migration `0034` backfilled them onto the roster first. `psa_drafting` stays, because whose counsel holds the pen is a fact about the transaction rather than an attribute of a firm, and it is answered on a Phase 1 row months before counsel is retained. Banner image path (`banner_image_path`) is the Vercel Blob pathname used in generated PDF headers. Note: no `phase`/`status` column. Phase is implicit in the checklist (each item lives in a phase, and the sidebar derives a "current phase" chip from incomplete items).
...
- **`checklist_items`** — leaf rows. Columns: `category_id`, `name`, `description` (canonical text from the template), `notes` (user-authored working notes, separate field on purpose), `optional`, `sort_order`, `completed`, `completed_at`, `completed_by`, `tracked_date` (date-only milestone field, surfaced only when the template flags the item with `dateField: true`).
  - **Milestone dates**: `estimated_date` is the date a milestone is projected for; `tracked_date` is the date it ACTUALLY landed. Migration `0035` added `estimated_date`; `0037` then moved every existing date out of `tracked_date` into `estimated_date`, because the dates on file were projections rather than outcomes, leaving `tracked_date` to fill in as milestones land. Two nullable `date` columns, surfaced as ONE date plus an `Est.` checkbox since 2026-09-07. The checkbox decides the column: ticked writes `estimated_date` and clears `tracked_date`; unticked writes `tracked_date` and leaves `estimated_date` frozen. Both columns stayed because the gap between them is the only record of slip, and freezing the projection on promotion captures it without anyone maintaining two fields. Read side is `tracked_date ?? estimated_date`, which is what every consumer already did, so the UI change needed no migration and moved no data. `tracked_date` kept its name to avoid a rename-vs-drop migration. Surfaced only on items the template flags `dateField: true` (ten items across Phases 2 to 4).
...
- **`issues`** (`src/db/schema/issues.ts`) — Phase 4 issues tracker. Columns: `deal_id`, `title`, `description`, `status` (`issue_status` enum), `priority` (`issue_priority` enum), `assignee_team_member_id` (nullable FK to `deal_team_members`, `ON DELETE SET NULL`), `identified_at`, `resolved_at`. The assignee is any Deal Team member (user, contact, or free-text identity source); display name is resolved via `src/lib/deal-team-name.ts::resolveDealTeamMemberName` at read time. Server actions validate the assignee's dtm id belongs to the same deal + org before write.
...
- **`audit_log`** (`src/db/schema/audit-log.ts`) — append-only audit trail. Columns: `org_id`, `user_id`, `action`, `entity_type`, `entity_id`, `before jsonb`, `after jsonb`, `metadata jsonb`. Only `created_at`, no `updated_at`. Read at `/admin/audit` (owner-gated; see `docs/features.md`). Written via the `writeAudit()` helper at `src/lib/audit.ts` (fire-and-forget with swallowed errors).
  ...
  **Naming convention.** `action` is `"<domain>.<past_tense_verb>"`; `entity_type` is the table the row lives in. These deliberately do not have to match, and several pairs don't (`member.password_reset` / `user`, `profile.updated` / `user`, `feedback.status_changed` / `feedback_item`). The action prefix names the domain a person thinks in, which is what reads well in the viewer's Action filter; `entity_type` names the table, which is what makes `entity_id` meaningful. Reviewers reliably read this as an inconsistency: it isn't, and renaming an action string would orphan every row already written under the old one.

  **`auditSafely()`.** `writeAudit` swallows its own errors, but that covers the INSERT only. A snapshot SELECT running *before* a mutation is already safe (a throw means the mutation never happens, which fails closed). A read running *after* the mutation has committed is not: it would make the action throw for a write that already landed, so the caller reports a failure for something that succeeded. Post-mutation audit work therefore goes inside `auditSafely(async () => { ... })`. Do not wrap pre-mutation snapshots in it: that would swallow real errors and let the mutation proceed on stale assumptions.

  **Transactions.** `writeAudit` uses the module-level `db`, so a call placed inside a `db.transaction(...)` callback escapes the transaction and would log a rolled-back write as though it had succeeded. Every `writeAudit` call must sit *after* the transaction resolves. Six functions use transactions today (`addContact`, `addBuilderToDeal`, `bulkAddContactsToDeal`, `deleteBuilder`, `removeMember`, `resetMemberPassword`).

  **Payload discipline.** Free-text columns have no length cap in this schema, so every value reaching jsonb goes through `truncateForAudit()` (500 chars), on both sides of a `before` / `after` pair and in `metadata`. Label builders cap at 80. Arrays in `metadata` are capped with a truthful total alongside, since `audit_log` only ever grows. Never log passwords, hashes, tokens, session ids, or full email bodies.

  **Coverage.** Every action that mutates shared data writes an entry, across `deals/[id]/actions.ts`, `deals/[id]/banner-actions.ts`, `deals/[id]/document-actions.ts`, `deals/actions.ts`, `contacts/actions.ts`, `builders/actions.ts`, `profile/actions.ts`, `set-password/actions.ts`, `admin/actions.ts`, `admin/feedback/actions.ts` and `components/feedback/actions.ts`. Deliberate exclusions: per-user sidebar deal ordering (`reorder-actions.ts`), `setMyNotificationPreference`, and `submitFeedback` / `addFeedbackComment` / `recordFeedbackAttachment`, whose rows already carry author and timestamp. `editFeedbackComment` and the three feedback deletes ARE audited, because they destroy content the row's own attribution cannot recover.

  Because most updates in this codebase are blind (`db.update(...).where(...)` with no prior read), capturing `before` needs an added SELECT. `loadChecklistItemAuditContext()` in `deals/[id]/actions.ts` is the shape to copy: it joins through to `deals` in one round-trip so `metadata` gets `dealId` / `dealName` / `label`, and it yields a deal id read from the database rather than trusting client-supplied input.
