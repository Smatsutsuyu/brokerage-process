...
- **`checklist_items`** — leaf rows. Columns: `category_id`, `name`, `description` (canonical text from the template), `notes` (user-authored working notes, separate field on purpose), `optional`, `sort_order`, `completed`, `completed_at`, `completed_by`, `tracked_date` (date-only milestone field, surfaced only when the template flags the item with `dateField: true`).
  - **Milestone dates**: `estimated_date` is the date a milestone is projected for; `tracked_date` is the date it ACTUALLY landed. Migration `0035` added `estimated_date`; `0037` then moved every existing date out of `tracked_date` into `estimated_date`, because the dates on file were projections rather than outcomes, leaving `tracked_date` to fill in as milestones land. Two independent nullable `date` columns, not one date plus a toggle: a milestone carries both and the gap between them is the slip. `tracked_date` kept its name to avoid a rename-vs-drop migration. Surfaced only on items the template flags `dateField: true` (ten items across Phases 2 to 4).
