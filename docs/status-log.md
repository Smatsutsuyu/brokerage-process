...
- **Tightened the `isShareDdMaterialItem` matcher** (revised later the same day). The original `includes("due diligence material")` also matched "Create Index of Due Diligence Material", so that row rendered a Send button wired to the DD-folder template — a send affordance on a do-the-work step, mailing a body about the folder rather than the index. Now anchored on `"share due diligence material"`, which hits exactly one row (verified against every DD-adjacent name in the template). The Index row keeps its checkbox, notes, and link affordances.
